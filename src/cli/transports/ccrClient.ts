  ...
  constructor(
    ...
    this.eventUploader = new SerialBatchEventUploader<ClientEvent>({
      ...
      send: async (batch) => {
        ...
        if (!result.ok) {
          ...
          throw new RetryableError('client event POST failed', failedResult.retryAfterMs);
        }
      },
      baseDelayMs: 500,
      maxDelayMs: 30_000,
    ...
    this.internalEventUploader = new SerialBatchEventUploader<WorkerEvent>({
      ...
      send: async (batch) => {
        ...
        if (!result.ok) {
          const failedResult = result as { retryAfterMs?: number };
          throw new RetryableError('internal event POST failed', failedResult.retryAfterMs);
        }
      },
      baseDelayMs: 500,
      maxDelayMs: 30_000,
    ...
    this.deliveryUploader = new SerialBatchEventUploader<{
      ...
      send: async (batch) => {
        ...
        if (!result.ok) {
          const failedResult = result as { retryAfterMs?: number };
          throw new RetryableError('delivery POST failed', failedResult.retryAfterMs);
        }
      },
      baseDelayMs: 500,
      maxDelayMs: 30_000,
