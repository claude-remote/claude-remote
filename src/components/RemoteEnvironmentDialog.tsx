...
import { LoadingState as LoadingStateIndicator } from './design-system/LoadingState.js';
const DIALOG_TITLE = 'Select Remote Environment';
const SETUP_HINT = 'Configure environments at: https://claude.ai/code';
type Props = {
  onDone: (message?: string) => void;
};
type DialogLoadingState = 'loading' | 'updating' | null;
export function RemoteEnvironmentDialog(t0) {
  const $ = _c(27);
  const { onDone } = t0;
  const [loadingState, setLoadingState] = useState<DialogLoadingState>('loading');
  let t1;
  ...
  if (loadingState === 'loading') {
    ...
    if ($[6] === Symbol.for('react.memo_cache_sentinel')) {
      t5 = <LoadingStateIndicator message={'Loading environments\u2026'} />;
      $[6] = t5;
    } else {
      t5 = $[6];
    }
  ...
  if (
    ...
    t5 =
      loadingState === 'updating' ? (
        <LoadingStateIndicator message={'Updating\u2026'} />
      ) : (
        <Select
