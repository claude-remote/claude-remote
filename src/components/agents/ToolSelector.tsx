...
function getToolBuckets(): ToolBuckets {
  return {
    ...
    EXECUTION: {
      ...
      toolNames: new Set(
        [BashTool.name, ('external' as string) === 'ant' ? TungstenTool.name : undefined].filter(
          (n) => n !== undefined,
        ),
      ),
    },
...
export function ToolSelector(t0) {
  ...
  if ($[22] !== selectedSet) {
    t9 = (bucketTools) => {
      const typedBucketTools = bucketTools as Array<{ name: string }>;
      const selected = count(typedBucketTools, (t_5) => selectedSet.has(t_5.name));
      const needsSelection = selected < typedBucketTools.length;
      return () => {
        const toolNames_1 = typedBucketTools.map(_temp4);
  ...
  if (
    ...
    bucketConfigs.forEach((t11) => {
      ...
      const selected_0 = count(bucketTools_0 as Array<{ name: string }>, (t_8) =>
