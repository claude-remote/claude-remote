...
function ElicitationFormDialog({
  ...
  useInput(
    (_input, key) => {
      ...
      const { schema: schema_5, name: name_0 } = currentField as {
        name: string;
        schema: PrimitiveSchemaDefinition & Record<string, any>;
      };
        ...
        {schemaFields.slice(scrollWindow.start, scrollWindow.end).map((field_0, visibleIdx) => {
          ...
          if (isMultiSelectEnumSchema(displaySchema)) {
            ...
            if (isExpanded) {
              ...
              if (selected_1.length > 0) {
                const displayLabels = selected_1.map((v_4) =>
                  getMultiSelectLabel(displaySchema, v_4),
                );
                valueContent = (
                  <Text>
                    {arrow}
            ...
            if (isActive) {
              ...
              const displayValue =
                hasValue && isDateTimeSchema(displaySchema)
                  ? formatDateDisplay(String(value_3), displaySchema)
                  : String(value_3);
              valueContent = hasValue ? (
