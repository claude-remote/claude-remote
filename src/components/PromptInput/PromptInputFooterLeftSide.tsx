...
function ModeIndicator({
  ...
  const hasTmuxSession = useAppState(
    (s_4) => IS_ANT_BUILD && s_4.tungstenActiveSession !== undefined,
  );
  const nextTickAt = useSyncExternalStore(
    proactiveModule?.subscribeToProactiveChanges ?? NO_OP_SUBSCRIBE,
    proactiveModule?.getNextTickAt ?? NULL,
  ...
  const voiceState = feature('VOICE_MODE')
    ...
    : ('idle' as const);
  const voiceWarmingUp = feature('VOICE_MODE') ? useVoiceState((s_6) => s_6.voiceWarmingUp) : false;
  ...
  const parts = [
    ...
    ...(IS_ANT_BUILD && hasTmuxSession
      ? [
          <Text key="tmux" color={tmuxSelected ? 'warning' : undefined}>
            tmux
          </Text>,
        ]
      : []),
