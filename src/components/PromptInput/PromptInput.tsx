...
function PromptInput({
  ...
  const hasTungstenSession = useAppState(
    (s) => IS_ANT_BUILD && s.tungstenActiveSession !== undefined,
  );
  ...
  const hasBgTaskPill = useMemo(
    () =>
      Object.values(tasks).some(
        (t) => isBackgroundTask(t) && !(IS_ANT_BUILD && isPanelAgentTask(t)),
      ),
    [tasks],
  );
  ...
  const tasksFooterVisible =
    (runningTaskCount > 0 || (IS_ANT_BUILD && coordinatorTaskCount > 0)) &&
    ...
    async (inputParam: string, isSubmittingSlashCommand = false) => {
      ...
      if (isAgentSwarmsEnabled()) {
        ...
        if (directMessage) {
          ...
          if ((result as { success: false; error: string }).error === 'no_team_context') {
  ...
  if (feature('QUICK_SEARCH')) {
    ...
    if (showQuickOpen) {
      return (
        <QuickOpenDialog onDone={() => setShowQuickOpen(false)} onInsert={insertWithSpacing} />
      );
    }
    if (showGlobalSearch) {
      return (
